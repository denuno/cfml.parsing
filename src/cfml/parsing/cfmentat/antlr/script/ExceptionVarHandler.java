...
public class ExceptionVarHandler{
  ...
  public void setExceptionVariable( cfCatchData _catchData ) throws cfmRunTimeException {
    if ( varname.length() == 0 )
      return;
    ...
    if ( !context.isCallEmpty() ){ // if there is a local scope
      ...
      if ( context.containsLocal( varname ) ){
        ...
        // throw an exception if another local variable of the same name exists that
        // is not one created by a parent catch block
        if ( !( val instanceof cfCatchData ) ){
          throw new CFException( "The local variable " + varname + " cannot be declared twice.", context );
        }else{
  ...
  public void deleteExceptionVariable() throws cfmRunTimeException {
    ...
    if ( preExistRef != null ){ // restore pre-existing local variable
      ...
      if ( !context.isCallEmpty() ){
        ...
        }catch( cfmRunTimeException ignored ){} // var should exist
      }else{
        cfStructData varScope = (cfStructData) session.getQualifiedData( variableStore.VARIABLES_SCOPE );
