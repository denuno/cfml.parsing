...
public class GenericStartTagTypeCf extends StartTagTypeGenericImplementation {
	...
	protected GenericStartTagTypeCf(final String description, final String startDelimiter,
			final String closingDelimiter, final EndTagType correspondingEndTagType, final boolean isServerTag) {
	...
	protected GenericStartTagTypeCf(final String description, final String startDelimiter,
			final String closingDelimiter, final EndTagType correspondingEndTagType, final boolean isServerTag,
			final boolean hasAttributes, final boolean isNameAfterPrefixRequired) {
	...
	protected void msg(String Msg, int TokenPos, int InputPos, String Extra) {
		System.out.println(Msg + "\t\t(TokenPos=" + String.valueOf(TokenPos) + ") (InputPos="
				+ String.valueOf(InputPos) + ")");
		if (Extra.length() > 0)
	...
	// Parses a series of name=value attributes and returns an array of
	// name,value pairs.
	protected ArrayList parseAttr(String Input, int StartLine, int StartPos) {
		 ...
		 * System.out.println("\\\\Tokens:"); for (int i=0;i<TokenMax;i++) {
		 * System.out.println(i+":"+Tokens[i]); }
		 * System.out.println("//tokens;"); System.out.println("");
		 */
		...
		while (InputPos < InputMax && TokenPos < TokenMax) {
			...
			if (InputPos < InputMax && TokenPos < TokenMax) {
				// Grabs the name - assumes names only contain word-characters
				// (alnum+underscore).
				// TODO: FIX: This doesn't accept the prefix:name format which
				// custom tags support.
				...
				// DEBUG:
				// msg("grabbed name",TokenPos,InputPos,"NAME:"+AttrName);
				...
				// If there's an equal sign, we've got name=value, consume and
				// continue...
				...
				// else we've just got name so value is set true
					...
					// DEBUG:
					// msg("No Value,Set True",TokenPos,InputPos,AttrName);
					AttrValue = "true";
				}

				// Skip past any whitespace tokens
				// TODO: CHECK: Can this go inside either of the IFs above/below
				// (not needed for name-only attrs?)
				while (InputPos < InputMax && TokenPos < TokenMax && isWhitespace(Tokens[TokenPos])) {
					LinePos += countNewlines(Tokens[TokenPos]);
					InputPos += Tokens[TokenPos].length();
					TokenPos++;
				...
				// If we don't know what the value is (i.e. not a name-only) and
				// hasn't hit the end, keep going.
				if (InputPos < InputMax && TokenPos < TokenMax && AttrValue == "") {
					...
					if (Tokens[TokenPos].equals("(")) {
						ModeStack.push("PAREN");
						TokenPos++;
						InputPos++;
					}
					if (Tokens[TokenPos].equals("{")) {
						ModeStack.push("BRACE");
						TokenPos++;
						InputPos++;
					}
					if (Tokens[TokenPos].equals("[")) {
						ModeStack.push("BRACKET");
						TokenPos++;
						InputPos++;
					}
					// DEBUG: msg(">>ModeStack",ModeStack.toString());

					// No mode, must be unquoted/unhashed/unbracketed, so value
					// stops at first whitespace (or the end).
					if (ModeStack.isEmpty()) {
						...
						// Until the modestack is empty (or we reach the end),
						// keep eating tokens building the value.
						while (InputPos < InputMax && TokenPos < TokenMax && (!ModeStack.isEmpty())) {
							CurMode = ModeStack.peek();

							// Since we checked the mode before we started,
							// don't repeat it for the first iteration.
							if (FirstLoop) {
								FirstLoop = false;
							} else {
								// Jump into next mode, if necessary.
								if (!CurMode.equals("DOUBLE") && !CurMode.equals("SINGLE")
										&& Tokens[TokenPos].equals("\"")) {
									ModeStack.push("DOUBLE");
									TokenPos++;
									InputPos++;
								}
								if (!CurMode.equals("SINGLE") && !CurMode.equals("DOUBLE")
										&& Tokens[TokenPos].equals("'")) {
									ModeStack.push("SINGLE");
									TokenPos++;
									InputPos++;
								}
								if (!CurMode.equals("HASH") && Tokens[TokenPos].equals("#")) {
									ModeStack.push("HASH");
									TokenPos++;
									InputPos++;
								}
							...
							if (CurMode.equals("DOUBLE")) {
								// All the while we haven't found a " or # we
								// can just keep eating tokens...
								while (TokenPos < TokenMax && !Tokens[TokenPos].equals("\"")
										&& !Tokens[TokenPos].equals("#")) {
									// DEBUG:
									// msg("running double",TokenPos,InputPos);
									...
									// (I think it's only needed for
									// attr="val""ue" - nested ones will be ok)
									while (TokenPos + 1 < TokenMax && Tokens[TokenPos].equals("\"")
											&& Tokens[TokenPos + 1].equals("\"")) {
										AttrValue += Tokens[TokenPos];
										LinePos += countNewlines(Tokens[TokenPos]);
										InputPos += Tokens[TokenPos].length();
										TokenPos++;
							...
							// If we're currently inside 'single quotes'...
								...
								while (TokenPos < TokenMax && !Tokens[TokenPos].equals("'")
										&& !Tokens[TokenPos].equals("#")) {
									// DEBUG:
									// msg("running single",TokenPos,InputPos);
									...
									// This ugly bit handles escaped quotes...
									// Again, probably only needed for
									// attr='val''ue' because of the empty stack
									// check.
									while (TokenPos + 1 < TokenMax && Tokens[TokenPos].equals("'")
											&& Tokens[TokenPos + 1].equals("'")) {
										AttrValue += Tokens[TokenPos];
							...
							// For anything not a string, we just keep going
							// until we find a 'special' character.
							else // any mode
							{
								while (TokenPos < TokenMax && !Tokens[TokenPos].equals("\"")
										&& !Tokens[TokenPos].equals("'") && !Tokens[TokenPos].equals("#")
										&& !Tokens[TokenPos].equals("(") && !Tokens[TokenPos].equals("{")
										&& !Tokens[TokenPos].equals("[") && !Tokens[TokenPos].equals(")")
										&& !Tokens[TokenPos].equals("}") && !Tokens[TokenPos].equals("]")) {
									// DEBUG:
									// msg("running multi/"+CurMode,TokenPos,InputPos,"val="+AttrValue);
							...
							// DEBUG:
							// msg("grabbed value",TokenPos,InputPos,"VAL:"+AttrValue);

							// If we hit a closing character, pop the mode off
							// the stack.
							if (CurMode.equals("DOUBLE") && Tokens[TokenPos].equals("\"")) {
								ModeStack.pop();
								TokenPos++;
								InputPos++;
							}
							if (CurMode.equals("SINGLE") && Tokens[TokenPos].equals("'")) {
								ModeStack.pop();
								TokenPos++;
								InputPos++;
							}
							...
							if (CurMode.equals("PAREN") && Tokens[TokenPos].equals(")")) {
								ModeStack.pop();
								TokenPos++;
								InputPos++;
							}
							if (CurMode.equals("BRACE") && Tokens[TokenPos].equals("}")) {
								ModeStack.pop();
								TokenPos++;
								InputPos++;
							}
							if (CurMode.equals("BRACKET") && Tokens[TokenPos].equals("]")) {
								ModeStack.pop();
								TokenPos++;
								InputPos++;
							}
	...
	// Parses any tag containing an expression instead of name=value pairs. In
	// particular, cfset and cfif.
	...
	/**
	 * <code>stripAttributes</code> - Strips the attributes from some data and
	 * puts them into a hash map
	 * 
	 * @param inData
	 *            - the string data to get the attributes out of
	 * @param match
	...
	protected ArrayList stripAttributes(String inData, int lineNum, int offset, ParseItemMatch match, String tagName) {
		...
		boolean isExpressionTag = (tagName.contains("cfset") || tagName.contains("cfif")
				|| tagName.contains("cfelseif") || tagName.contains("cfreturn") || tagName.contains("cfwhile"));
